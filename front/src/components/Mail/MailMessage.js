...
export default function MailMessage() {
  ...
  const messageSend = () =>{
    ...
    if ( Object.keys(personContact).length === 0 ){
      ...
      axios.post(`https://localhost:3306/mail/message/${authState.id}`, body, head).then((res)=>{
          if (res.data.error){
  ...
  const handleChange = (event) => {
    ...
    axios.get(`https://localhost:3306/mail/auctions/${authState.id}`, head).then((res)=>{
...
useEffect(()=>{
  ...
  axios.get(`https://localhost:3306/mail/contacts/${authState.id}`, head).then((res)=>{
    console.log(res.data);
    setMails(res.data);
