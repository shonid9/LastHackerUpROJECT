...
export default function MailOutbox() {
  ...
  useEffect(()=>{
      ...
      axios.get(`https://localhost:3306/mail/outbox/${authState.id}`, head).then((res)=>{
  ...
  const displayMessage = (name) =>{
      axios.get(`https://localhost:3306/auth/fetchy/${name.senderId}`).then((respo)=>{
  ...
  const deleteMessage = (name) =>{
        ...
        axios.put(`https://localhost:3306/mail/message/deleteoutbox/${name.id}`).then((res)=>{
        });
        let mails = [...mailList];
