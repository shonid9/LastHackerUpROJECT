...
export default function MailInbox() {
  ...
  useEffect(()=>{
      ...
      axios.get(`https://localhost:3306/mail/inbox/${authState.id}`, head).then((res)=>{
  ...
  const displayMessage = (name) =>{
    ...
    if (name.read===false){
      ...
      axios.put(`https://localhost:3306/mail/message/read/${name.id}`).then((res)=>{
      });
  ...
  const deleteMessage = (name) =>{
    ...
    axios.put(`https://localhost:3306/mail/message/deleteinbox/${name.id}`).then((res)=>{
    });
    let mails = [...mailList];
